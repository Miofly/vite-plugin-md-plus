...
import type { MarkdownItFigureOptions } from './options';
...
export const figure: PluginWithOptions<MarkdownItFigureOptions> = (md, options = {}) => {
  md.core.ruler.before('linkify', 'figure', state => {
    ...
    for (let index = 1, { length } = state.tokens; index < length - 1; index++) {
      ...
      if (index !== 0 && state.tokens[index - 1].type !== 'paragraph_open') continue;

      // next token is paragraph close
      if (index !== length - 1 && state.tokens[index + 1].type !== 'paragraph_close') continue;
