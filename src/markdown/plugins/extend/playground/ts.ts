...
export const getTSPlaygroundPreset = ({ service = 'https://www.typescriptlang.org/play', ...compilerOptions }: TSPresetPlaygroundOptions = {}): PlaygroundOptions => ({
  name: 'playground#ts',
  propsGetter: ({ title = '', files, settings, key }: PlaygroundData): Record<string, string> => {
    ...
    const link = `${service}${getURL(files[tsfiles[0]].content, deepAssign({}, <CompilerOptions>settings || {}, <CompilerOptions>compilerOptions))}`;
